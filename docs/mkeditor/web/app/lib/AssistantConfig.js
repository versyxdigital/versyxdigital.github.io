...
class AssistantConfig {
    ...
    static update(request) {
        ...
        switch (request.provider) {
            case 'anthropic':
                merged.anthropic = AssistantConfig.mergeApi(merged.anthropic, request.config);
                break;
            case 'openai':
                merged.openai = AssistantConfig.mergeApi(merged.openai, request.config);
                break;
            case 'ollama':
                merged.ollama = AssistantConfig.mergeOllama(merged.ollama, request.config);
                break;
        }
        store.providers = merged;
        return (0, assistantStoreFile_1.writeAssistantStore)(store) ? merged : null;
    }
    /**
     ...
     */
    static withDefaults(partial) {
        return {
            anthropic: {
                ...Assistant_1.DEFAULT_PROVIDER_CONFIG.anthropic,
                ...(partial.anthropic ?? {}),
            },
            openai: {
                ...Assistant_1.DEFAULT_PROVIDER_CONFIG.openai,
                ...(partial.openai ?? {}),
            },
            ollama: {
                ...Assistant_1.DEFAULT_PROVIDER_CONFIG.ollama,
                ...(partial.ollama ?? {}),
            },
        };
    }
    static mergeApi(current, patch) {
        return { ...current, ...AssistantConfig.pickDefined(patch) };
    }
    static mergeOllama(current, patch) {
        return { ...current, ...AssistantConfig.pickDefined(patch) };
    }
    /**
